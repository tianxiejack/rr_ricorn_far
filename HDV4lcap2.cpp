...
void YUYVEnhance(unsigned char * dst,unsigned char * src,int w,int h)
{
		...
		static unsigned char * gpu_uyvy[2];
		static unsigned char * gpu_rgb[2];
		static unsigned char * gpu_enh[2];
		static unsigned char * cpu_uyvy[2];
		static bool once =true;
		static cudaStream_t m_cuStream[2];
		if(once)
		{
			for(int i=0; i<2; i++){
			cudaMalloc((void **)&gpu_uyvy[i],FPGA_SINGLE_PIC_W*FPGA_SINGLE_PIC_H*6*2);
			cudaMalloc((void **)&gpu_rgb[i],FPGA_SINGLE_PIC_W*FPGA_SINGLE_PIC_H*6*3);
			cudaMalloc((void **)&gpu_enh[i],(FPGA_SINGLE_PIC_W)*(FPGA_SINGLE_PIC_H*6)*3);
				cudaStreamCreate(&m_cuStream[i]);
				cpu_uyvy[i]=(unsigned char *)malloc(FPGA_SINGLE_PIC_W*FPGA_SINGLE_PIC_H*6*2);
			}
			once=false;
		...
		cudaMemcpy(gpu_uyvy[fourOrsix],src,noww*nowh*2,cudaMemcpyHostToDevice);
		//yuyv2bgr_(gpu_rgb,gpu_yuyv,SDI_WIDTH,SDI_HEIGHT,m_cuStream[0]);
		uyvy2bgr_(gpu_rgb[fourOrsix],gpu_uyvy[fourOrsix],noww,nowh,m_cuStream[fourOrsix]);
		Mat dst1(nowh,noww,CV_8UC3,gpu_enh[fourOrsix]);
		Mat src1(nowh,noww,CV_8UC3,gpu_rgb[fourOrsix]);
